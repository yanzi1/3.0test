...
public class StorageUtils {
    ...
    public static Observable<List<StorageBean>> getObservableStorageBean(final Context context) {
        return Observable.create(new ObservableOnSubscribe<List<StorageBean>>() {
            @Override
            public void subscribe(ObservableEmitter<List<StorageBean>> emitter)
                    throws Exception {
                emitter.onNext(getStorageData(context.getApplicationContext()));
                emitter.onComplete();
            }
