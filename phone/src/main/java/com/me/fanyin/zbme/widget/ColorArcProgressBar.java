...
public class ColorArcProgressBar extends View {
    ...
    private Paint vTextPaint;//当前正确率画笔
    private Paint percentUnitPaint;//百分号画笔
    ...
    private float textSize = dipToPx(55);
    ...
    private String longDegreeColor = "#CCCCCC";
    private String shortDegreeColor = "#111111";
    private String bgArcColor = "#CCCCCC";
    ...
    private void initCofig(Context context, AttributeSet attrs) {
        ...
        bgColor = a.getColor(R.styleable.ColorArcProgressBar_back_color,Color.GRAY);
    ...
    private void initView() {
        ...
        //百分比符号
        percentUnitPaint = new Paint();
        percentUnitPaint.setTextSize(textSize/4);
        percentUnitPaint.setColor(Color.BLACK);
        percentUnitPaint.setTextAlign(Paint.Align.CENTER);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isNeedContent) {
            float curValuesTextWidth = getTextWidth(vTextPaint,String.format("%.0f", curValues));
            canvas.drawText(String.format("%.0f", curValues), centerX, centerY + textSize / 3 - getTextHeight(hintPaint), vTextPaint);
            canvas.drawText("%", centerX + curValuesTextWidth/2 , centerY + textSize / 3 - getTextHeight(hintPaint), percentUnitPaint);
        }
        if (isNeedUnit) {
            canvas.drawText(hintString, centerX, centerY + 2 * textSize / 3 - getTextHeight(hintPaint), hintPaint);
        }
        if (isNeedTitle) {
    ...
    //精确计算文字的宽度：
    public float  getTextHeight(Paint mPaint)
    {
