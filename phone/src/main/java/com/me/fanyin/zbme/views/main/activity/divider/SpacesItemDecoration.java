...
public class SpacesItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public SpacesItemDecoration(int leftRight, int topBottom,@ColorInt int mColor) {
...
//    }
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        int childPosition = parent.getChildAdapterPosition(view);
            int totalCount = layoutManager.getItemCount(); //header

            //判断总的数量是否可以整除
            int surplusCount = (totalCount) % layoutManager.getSpanCount(); //剩余行


            if (surplusCount == 0 && childPosition > totalCount - layoutManager.getSpanCount() ) {
                //后面几项需要bottom
