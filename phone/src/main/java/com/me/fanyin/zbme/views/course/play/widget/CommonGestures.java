 ...
 * Copyright (c) 2014 XunIce <xunice@qq.com>
 *
	...
	private class DoubleTapGestureListener extends SimpleOnGestureListener {
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
			if (mListener != null && mGestureEnabled && e1 != null && e2 != null) {
				...
				int windowWidth = DensityUtils.getWidthInDp(mContext);
				int windowHeight = DensityUtils.getHeightInDp(mContext);
				if (Math.abs(e2.getY(0) - mOldY) * 2 > Math.abs(e2.getX(0) - mOldX)) {
					if (mOldX > windowWidth/ 2+150) {
						mListener.onRightSlide((mOldY - e2.getY(0)) / windowHeight);
					} else if (mOldX < windowWidth / 2-150) {
						mListener.onLeftSlide((mOldY - e2.getY(0)) / windowHeight);
					}
				}else{
					if ((e2.getX()-e1.getX()) > 0) {
						mListener.forward(e2.getX()-e1.getX());
					} else if ((e2.getX()-e1.getX()) < 0) {
						mListener.backward(e1.getX()-e2.getX());
					}
				}
			}
