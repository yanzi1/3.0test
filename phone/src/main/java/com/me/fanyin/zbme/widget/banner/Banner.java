package com.me.fanyin.zbme.widget.banner;
...
public class Banner extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    private int mIndicatorWidth = BannerConfig.INDICATOR_SIZE_WIDTH;
    private int mIndicatorHeight = BannerConfig.INDICATOR_SIZE_HEIGHT;
    ...
    private int defaultImage=R.mipmap.img_default_banner;
    private int count=0;
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        mIndicatorWidth =typedArray.getDimensionPixelSize(R.styleable.Banner_indicator_width, BannerConfig.INDICATOR_SIZE_WIDTH);
        mIndicatorHeight =typedArray.getDimensionPixelSize(R.styleable.Banner_indicator_height, BannerConfig.INDICATOR_SIZE_HEIGHT);
...
//            iv.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
            View itemView = LayoutInflater.from(getContext()).inflate(R.layout.banner_xml, null, false);
            ...
            if(imageListener!=null){
                ...
                //默认图片
                    Glide.with(context).load(url).crossFade().fitCenter().placeholder(defaultImage).into(iv);
//                    Glide.with(context).load(url).crossFade().placeholder(R.mipmap.img_default_detail).into(iv);
            }
        }
        setData();
    }
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if(imageListener!=null){
                ...
                if (defaultImage==-1)
                    ...
                    Glide.with(context).load(url).centerCrop().crossFade().fitCenter().placeholder(defaultImage).into(iv);
//                    Glide.with(context).load(url).centerCrop().crossFade().placeholder(R.mipmap.img_default_detail).into(iv);
            }
        }
        setData();
    }
    ...
    private static class MyRunnable implements Runnable{
        ...
        public MyRunnable(Banner banner) {
            weakReference = new WeakReference<Banner>(banner);
        }
        ...
        public void run() {
            ...
            if (banner.isAutoPlay) {
                if (banner.count>1) {
                    banner.currentItem = banner.currentItem % (banner.count + 1) + 1;
                    if (banner.currentItem == 1) {
                        banner.viewPager.setCurrentItem(banner.currentItem, false);
                    } else {
                        banner.viewPager.setCurrentItem(banner.currentItem);
                    }
                    banner.handler.postDelayed(banner.task, banner.delayTime);
                }
            }
        }
    }
    ...
    class BannerPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            if(imageViews.get(position).getParent() != null)
                ((ViewGroup)imageViews.get(position).getParent()).removeAllViews();
