...
class SmartTabStrip extends LinearLayout {
	...
	private void drawDecoration(Canvas canvas) {
		...
		if (tabCount > 0) {
			...
			float padding = (Math.abs(selectedStart - selectedEnd) - indicatorWidth) / 2f;
			float left, right;
			if (isLayoutRtl) {
				if (indicatorWidth == AUTO_WIDTH) {
					...
					left = selectedEnd + padding;
					right = selectedStart - padding;
				}
			} else {
				if (indicatorWidth == AUTO_WIDTH) {
					left = selectedStart;
					...
					left = selectedStart + padding;
					right = selectedEnd - padding;
				}
			}
			...
			if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
				...
				if (indicatorWidth == AUTO_WIDTH) {
					...
					nextStart = Utils.getStart(nextTab, indicatorWithoutPadding) + padding;
					nextEnd = Utils.getEnd(nextTab, indicatorWithoutPadding) - padding;
				}
				if (isLayoutRtl) {
	...
	private void drawIndicator(Canvas canvas, float left, float right, float height, float thickness,
							   int color) {
