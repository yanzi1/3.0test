...
public class ImagePickerAdapter extends RecyclerView.Adapter<ImagePickerAdapter.SelectedPicViewHolder> {
    ...
    public SelectedPicViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new SelectedPicViewHolder(mInflater.inflate(R.layout.course_note_image, parent, false));
    }
    ...
    public class SelectedPicViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void bind(int position) {
            ...
            if (isAdded && position == getItemCount() - 1) {
                iv_img.setPadding(40,40,40,40);
//                iv_img.setScaleType(ImageView.ScaleType.CENTER_CROP);
                iv_img.setImageResource(R.mipmap.note_photo);
                clickPosition = NotesActivity.IMAGE_ITEM_ADD;
            } else {
                iv_img.setPadding(0,0,0,0);
//                iv_img.setScaleType(ImageView.ScaleType.MATRIX);
                if(item.path.startsWith("http")){
                    ImagePicker.getInstance().getImageLoader().displayImage((Activity) mContext, item.path, iv_img);
                }else{
