...
//            release(true);
    ...
    public void setVideoLayout(int layout, float aspectRatio) {
        ...
        if (mContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) { //初始化Controller高度
            if (SharedPrefHelper.getInstance().getMiniScreenHight() == -1) {
                SharedPrefHelper.getInstance().setMiniScreenHight(lp.height);
            }
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, lp.height);
            mMediaController.setLayoutParams(layoutParams);
            setLayoutParams(lp);
        } else {
            if (SharedPrefHelper.getInstance().getFullScreenHight() != -1) {
                lp.height = SharedPrefHelper.getInstance().getFullScreenHight();
            }
...
//    屏幕“暗”，表示目前屏幕是黑的 。
    public void start() {
        ...
        if(mMediaController!=null){
            if(!mMediaController.playActivity.isServiceStart){
        ...
        if (mMediaController != null && mMediaController.playActivity!=null) {
            if (!mMediaController.playActivity.isCanPlay) {
                return;
            }
        }
